...
struct SpoOptimizationSummary {
    ...
    temp_loss_sum: f64,
    alpha_loss_sum: f64,
    mpo_total_loss_sum: f64,
    grad_norm_sum: f64,
    optimization_updates: usize,
...
impl SpoOptimizer {
    ...
    fn optimize_update<B: AutodiffBackend>(
        ...
        if replay.can_sample(args.sample_sequence_length) {
            for _ in 0..args.epochs {
                for _ in 0..args.num_minibatches {
                    ...
                    let sampled_obs_refs = sampled.obs.iter().map(|obs| obs.as_ref()).collect::<Vec<_>>();
                    let sampled_next_obs_refs = sampled
                        .next_obs
                        .iter()
                        .map(|obs| obs.as_ref())
                        .collect::<Vec<_>>();
                    ...
                    let policy_logits = agent_online.actor_logits(
                        adapter.build_actor_input_batch(&sampled_obs_refs, args, obs_dim, device)?,
                    );
                    ...
                    agent_online.actor =
                        actor_optim.step(current_actor_lr, agent_online.actor.clone(), grads_actor);
                    agent_online.critic = critic_optim
                        .step(current_critic_lr, agent_online.critic.clone(), grads_critic);
                    *duals = dual_optim.step(args.dual_lr, duals.clone(), grads_duals);

                    soft_update_params::<Agent<B>, B>(agent_target, agent_online, args.tau);
        ...
        Ok(SpoOptimizationSummary {
            elapsed_ms: optimization_started.elapsed().as_secs_f64() * 1_000.0,
            actor_loss_sum,
            critic_loss_sum,
            temp_loss_sum,
            alpha_loss_sum,
            ...
            sample_duration_ms,
            forward_loss_duration_ms: model_forward_loss_duration_ms,
...
fn run_search_eval<B: AutodiffBackend>(
    ...
    run_eval_with_policy(
        ...
        |pool, cur_steps| {
            ...
            let root_state_ids = pool.snapshot(&eval_env_ids)?;
            ...
            pool.release_batch(&root_state_ids);
            pool.release_batch(&search_out.leaf_state_ids);

            Ok(greedy_actions_from_weights(
...
fn run_loop<B: AutodiffBackend>(
    ...
    let obs_dim = adapter.infer_obs_dim(&first_obs.obs, &args);
    ...
    let mut agent_online = Agent::<B>::new(
        obs_dim,
        args.hidden_dim,
        action_dim,
        adapter.uses_binpack_architecture(),
        &device,
    );
    ...
    for update in 0..args.num_updates {
        ...
        let optimization_summary = SpoOptimizer::optimize_update(
            &replay,
            &args,
            adapter.as_ref(),
            obs_dim,
        ...
        let optimization_elapsed = optimization_summary.elapsed_ms;

        if is_lead {
            let denom = (optimization_summary.optimization_updates as f64).max(1.0);
            let actor_loss_sum = optimization_summary.actor_loss_sum;
            let critic_loss_sum = optimization_summary.critic_loss_sum;
            let temp_loss_sum = optimization_summary.temp_loss_sum;
            let alpha_loss_sum = optimization_summary.alpha_loss_sum;
            let mpo_total_loss_sum = optimization_summary.mpo_total_loss_sum;
            ...
            let total_update_secs = (rollout_elapsed.as_secs_f64() + optimization_elapsed / 1_000.0)
                .max(1.0e-9);
            ...
            debug!(
                ...
                optimization_duration_ms = optimization_elapsed,
                optimization_sample_duration_ms = optimization_summary.sample_duration_ms,
...
pub struct SpoTrainer<'a, B: AutodiffBackend> {
    args: Args,
    dist: DistInfo,
...
impl<'a, B: AutodiffBackend> SpoTrainer<'a, B> {
...
pub fn run<B: AutodiffBackend>(
    ...
    SpoTrainer::<B>::new(args, dist, context, device).run()
}
