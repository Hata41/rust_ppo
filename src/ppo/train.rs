...
pub fn run<B: AutodiffBackend>(args: Args, dist: DistInfo, device: B::Device) -> Result<()> {
    ...
    if dist.rank == 0 {
        ...
        println!(
            "world_size={} global_num_envs={} local_num_envs={} rollout_length={} local_batch={}",
            dist.world_size, args.num_envs, local_num_envs, args.rollout_length, local_batch
        );
    ...
    for update in 0..args.num_updates {
        ...
        if dist.rank == 0 && update % 10 == 0 {
            ...
            println!(
                ...
                update, timesteps, mean_return, last_actor_loss, last_critic_loss, last_entropy
            );
        }
    }
